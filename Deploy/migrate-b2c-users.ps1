<#
.SYNOPSIS
    Migrates users from Azure B2C to Entra External ID for the dev environment.

.DESCRIPTION
    Phase 1: Exports all users from the B2C dev tenant via Microsoft Graph API.
...
.PARAMETER ExportFile
    Path to the export JSON file. Defaults to Deploy/b2c-dev-users-export.json.
...
.EXAMPLE
    ...
    .\migrate-b2c-users.ps1

    # Export only (review before importing)
    .\migrate-b2c-users.ps1 -ExportOnly

    # Import only (using existing export)
    .\migrate-b2c-users.ps1 -SkipExport

    # Dry run (see what would happen)
    .\migrate-b2c-users.ps1 -DryRun
#>

param(
    [switch]$SkipExport,
    [switch]$ExportOnly,
    [string]$ExportFile = "$PSScriptRoot/b2c-dev-users-export.json",
    [switch]$DryRun
)
...
$B2CTenantDomain = 'trashmobdev.onmicrosoft.com'
$EntraTenantId = '8577fa31-4b86-4e4b-8b02-93fba708cb19'
$EntraTenantDomain = 'trashmobecodev.onmicrosoft.com'
...
if (-not $SkipExport) {
    ...
    Write-Host " Phase 1: Export users from B2C" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host " Phase 2: Import users to Entra External ID" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "     https://entra.microsoft.com (switch to dev tenant)" -ForegroundColor Yellow
Write-Host "  2. After PR #2688 deploys, visit https://dev.trashmob.eco/api/config" -ForegroundColor Yellow
