...
export function getEventShareMessage(eventToShare: EventData, currUserId: string) {
    ...
    const eventTime = new Date(eventToShare.eventDate).toLocaleTimeString('en-us', {
        hour12: true,
        hour: 'numeric',
        minute: '2-digit',
    });
    ...
    if (currUserId === eventToShare.createdByUserId) {
        return (
            `Join my next {{TrashMob}} event on ${eventDate} at ${eventTime} in ${eventToShare.city}.\n` +
            `Sign up using the link for more details! Help me clean up ${eventToShare.city}!`
        );
    }
    return (
        `Join me at this {{TrashMob}} event on ${eventDate} at ${eventTime} in ${eventToShare.city}.\n` +
        `Sign up using the link for more details! Help me clean up ${eventToShare.city}!`
    );
}
...
export function getCancellationMessage(eventToShare: any, cancellationReason: string) {
    ...
    const message =
        `Sorry everyone, we had to cancel our {{TrashMob}} event ${eventToShare.name} in #${eventToShare.city} on ${eventDate}. ${cancellationReason}.\n` +
...
export function getEventSummaryMessage(city: string, numAttendees: number, numBags: number) {
    const message =
        `We just finished a {{TrashMob}} event in ${city}. ${numAttendees} attendees picked up ${numBags} bags of #litter. ` +
        'Sign up using the link to get notified the next time we are having an event. Help us clean up the planet!';
...
export function getEventDetailsMessage(eventDate: Date, city: any, createdById: string, currUserId: string) {
    ...
    if (createdById === currUserId) {
        return (
            `Join my next {{TrashMob}} event on ${eventDateFormatted} at ${eventTime} in ${city}.\n` +
            `Sign up using the link for more details! Help me clean up ${city}!`
        );
    }
    return (
        `Join me at this {{TrashMob}} event on ${eventDateFormatted} at ${eventTime} in ${city}.\n` +
        `Sign up using the link for more details! Help me clean up ${city}!`
    );
}

export const InvitationMessage =
    'Interested in cleaning up the planet? Check out {{TrashMob}}! ' +
