...
import { ChevronDown, CircleUserRound, CircleHelp, Gauge, IdCard, LogOut, Map, Plus, User, UserRoundX } from 'lucide-react'
...
export const UserNav = (props: UserNavProps) => {
	...
	return (
		<div className={cn("flex flex-row item-center basis-full lg:basis-0", className)}>
			{!isUserLoaded && <Button variant="outline" className="text-primary border-primary" onClick={signIn}>Sign in</Button>}
			{isUserLoaded && (
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button variant="outline">
							<CircleUserRound size={32} />{currentUser.userName || 'Welcome'}<ChevronDown />
						</Button>
					</DropdownMenuTrigger>
					<DropdownMenuContent>
						<DropdownMenuItem asChild>
							<Link to="/mydashboard">
								<Gauge /><span>My Dashboard</span>
							</Link>
						</DropdownMenuItem>
						<DropdownMenuItem asChild>
						...
						<DropdownMenuItem onClick={profileEdit}>
							<User /><span>Update my profile</span>
						</DropdownMenuItem>
						<DropdownMenuItem asChild>
							<Link to="/locationpreference">
								<Map /><span>My Location Preference</span>
							</Link>
						</DropdownMenuItem>
						{currentUser.isSiteAdmin && (
							<DropdownMenuItem asChild>
								<Link to="/siteadmin">
									<IdCard /><span>Site Administration</span>
								</Link>
							</DropdownMenuItem>
						)}
						<DropdownMenuItem asChild>
							<Link to="/deletemydata">
								<UserRoundX /><span>Delete my account</span>
							</Link>
						</DropdownMenuItem>
						<DropdownMenuSeparator />
