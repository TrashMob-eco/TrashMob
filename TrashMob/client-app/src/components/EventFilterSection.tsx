...
export const EventFilterSection: FC<EventFilterSectionProps> = ({
    ...
    const [passTimeFrame] = useState<string[]>(
        Object.values(EventTimeFrame).filter((t) => t.startsWith('Any') || t.startsWith('Past')),
    );
    const [futureTimeFrame] = useState<string[]>(
        Object.values(EventTimeFrame).filter((t) => t.startsWith('Any') || t.startsWith('Next')),
    );
    ...
    const handleCountryChange = useCallback(
        (selectedCountry: string) => {
            if (selectedCountry === '' || !locationMap.has(selectedCountry)) {
                ...
                const states: Map<string, Set<string>> = locationMap.get(selectedCountry) ||
    ...
    const handleStateChange = useCallback(
        (selectedState: string) => {
            const stateMap = locationMap.get(selectedCountry);
            if (stateMap) {
                if (selectedState === '') {
                    setSelectedState('');
                    setCities([]);
    ...
    return (
        <div className='d-flex'>
            ...
            />
            <button className='ml-1 btn-withoutline' hidden={!isEventFiltering} onClick={resetFilters}>
                Reset
            </button>
        </div>
    );
