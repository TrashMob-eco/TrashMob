...
export const EventSummaries: React.FC = () => {
  const [displaySummaries, setDisplaySummaries] = React.useState<
    DisplayEventSummary[]
  >([]);
  const [eventTypeList, setEventTypeList] = React.useState<EventTypeData[]>([]);
  const [isEventSummaryDataLoaded, setIsEventSummaryDataLoaded] =
  ...
  return (
    <>
      <div className="w-100">
        ...
        {renderSearchForm()}
        {isEventSummaryDataLoaded &&
          renderEventSummariesTable(displaySummaries)}
      </div>
    </>
  );
};
