...
interface EventRouteTimeTrimDialogProps {
...
export const EventRouteTimeTrimDialog = ({ route, open, onOpenChange, eventId }: EventRouteTimeTrimDialogProps) => {
    ...
    const restoreMutation = useMutation({
        mutationFn: async () => {
            if (!route) return;
            const result = await RestoreRouteTime({ routeId: route.id }).service();
            return result.data;
        },
        onSuccess: () => {
            invalidateQueries();
