...
export const MapControllerSinglePointNoEvent: React.FC<MapControllerProps> = (props) => {
    ...
    useEffect(() => {
        if (mapRef && props.isMapKeyLoaded && !isDataSourceLoaded && isMapReady) {
            ...
            const markerLayer = new HtmlMarkerLayer(dataSourceRef, 'marker1', {
                markerCallback: (id: any, position: data.Position, properties: any) => {
                    ...
                    mapRef.events.add('dragend', marker, (e: any) => {
                        const pos = e.target.options.position;
            ...
            const position = new data.Point(new data.Position(props.longitude, props.latitude));
    ...
    useEffect(() => {
        if (mapRef && props.isMapKeyLoaded && isDataSourceLoaded && isMapReady && !isPrevLoaded) {
            const dsr = mapRef.sources.getById('mainDataSource') as source.DataSource;
            const feature = dsr.getShapes()[0];

            let position = new data.Position(props.longitude, props.latitude);
    ...
    const handlePagination = (e: any, shownResults: any) => {
        ...
        makeAndHandleRequest(query, page).then((resp: any) => {
    ...
    const handleSearch = React.useCallback((q) => {
        ...
        setIsLoading(true);
        makeAndHandleRequest(q).then((resp: any) => {
            CACHE[q] = { ...resp, page: 1 };
    ...
    async function makeAndHandleRequest(query: string, page: number = 1) {
        const res = await azureMapSearchAddress.mutateAsync({
            azureKey: mapKeyRef.current,
            query,
        });
        const options = res.data.results.map((i: any) => ({
            id: i.id,
            displayAddress: i.address.freeformAddress,
            position: i.position,
        }));
        const { totalResults } = res.data.summary;
