...
export const PartnerSocialMediaAccounts: React.FC<PartnerSocialMediaAccountsDataProps> = (props) => {
    ...
    React.useEffect(() => {
        if (accountName === '' || accountNameErrors !== '') {
            setIsSaveEnabled(false);
        } else {
    ...
    const accountActionDropdownList = (accountId: string, accountName: string) => (
        <>
            <Dropdown.Item onClick={() => editAccount(accountId)}>
                <Pencil />
                Edit Document
            </Dropdown.Item>
            <Dropdown.Item onClick={() => removeAccount(accountId, accountName)}>
                <XSquare />
                Remove Document
            </Dropdown.Item>
        </>
    ...
    function renderPartnerSocialMediaAccountsTable(accounts: PartnerSocialMediaAccountData[]) {
        return (
            <div>
                            ...
                            <tr key={account.id.toString()}>
                                ...
                                <td>
                                    {getSocialMediaAccountType(
                                        socialMediaAccountTypeList,
                                        account.socialMediaAccountTypeId,
                                    )}
                                </td>
                ...
                <Button disabled={!isAddEnabled} className='action' onClick={() => addSocialMediaAccount()}>
                        ...
                        <Col>
                            <Form.Group className='required'>
                                <OverlayTrigger placement='top' overlay={renderSocialMediaAccountNameToolTip}>
                                    <Form.Label className='control-label font-weight-bold h5' htmlFor='AccountName'>
                                        AccountName
                                    </Form.Label>
                                </OverlayTrigger>
                                <Form.Control
                                    ...
                                    onChange={(val) => handleAccountNameChanged(val.target.value)}
                            ...
                            <Form.Group>
                                ...
                                <div>
                                    <select
                                        ...
                                        defaultValue={socialMediaAccountTypeId}
                                        onChange={(val) => selectSocialMediaAccountType(val.target.value)}
                                        required
                                    >
            ...
            <Row className='gx-2 py-5' lg={2}>
                <Col lg={4} className='d-flex'>
                    <div className='bg-white py-2 px-5 shadow-sm rounded'>
                        ...
                        <p>
                            This page allows you to add a list of social media accounts you would like to have tagged
                            when you approve a partnership request to both help spread the word about what TrashMob.eco
                            users are doing within your community, and how your organization is helping your community.
                            This feature is still in development, but adding the information when you set things up now
                            will help when this feature fully launches.
                        </p>
                    </div>
                </Col>
                <Col lg={8}>
                    <div className='bg-white p-5 shadow-sm rounded'>
                        ...
                        {isSocialMediaAccountsDataLoaded
                            ? renderPartnerSocialMediaAccountsTable(socialMediaAccounts)
                            : null}
                        {isEditOrAdd ? renderAddPartnerSocialMediaAccount() : null}
                    </div>
                </Col>
            </Row>
        </Container>
