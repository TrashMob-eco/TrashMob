...
export const PartnerDocuments: React.FC<PartnerDocumentsDataProps> = (props) => {
    ...
    React.useEffect(() => {
        if (documentName === '' || documentNameErrors !== '') {
            setIsSaveEnabled(false);
        } else {
    ...
    const documentActionDropdownList = (documentId: string, documentName: string) => (
        <>
            <Dropdown.Item onClick={() => editDocument(documentId)}>
                <Pencil />
                Edit Document
            </Dropdown.Item>
            <Dropdown.Item onClick={() => removeDocument(documentId, documentName)}>
                <XSquare />
                Remove Document
            </Dropdown.Item>
        </>
    ...
    function renderPartnerDocumentsTable(documents: PartnerDocumentData[]) {
        return (
            <div>
                ...
                <Button disabled={!isAddEnabled} className='action' onClick={() => addDocument()}>
                    Add Document
                </Button>
            </div>
        );
                        ...
                        <Col>
                            <Form.Group className='required'>
                                <OverlayTrigger placement='top' overlay={renderDocumentNameToolTip}>
                                    <Form.Label className='control-label font-weight-bold h5' htmlFor='DocumentName'>
                                        Document Name
                                    </Form.Label>
                                </OverlayTrigger>
                                <Form.Control
                                    ...
                                    onChange={(val) => handleDocumentNameChanged(val.target.value)}
                        ...
                        <Col>
                            <Form.Group>
                                <OverlayTrigger placement='top' overlay={renderDocumentUrlToolTip}>
                                    <Form.Label className='control-label font-weight-bold h5' htmlFor='DocumentUrl'>
                                        DocumentUrl
                                    </Form.Label>
                                </OverlayTrigger>
                                <div>
                                    <Form.Control
                                        ...
                                        onChange={(val) => handleDocumentUrlChanged(val.target.value)}
                                        maxLength={parseInt('64')}
                                        required
                                    />
                                </div>
            ...
            <Row className='gx-2 py-5' lg={2}>
                ...
                <Col lg={8}>
                    <div className='bg-white p-5 shadow-sm rounded'>
                        ...
                        {isPartnerDocumentsDataLoaded ? renderPartnerDocumentsTable(partnerDocuments) : null}
                        {isEditOrAdd ? renderAddDocument() : null}
                    </div>
                </Col>
            </Row>
        </Container>
