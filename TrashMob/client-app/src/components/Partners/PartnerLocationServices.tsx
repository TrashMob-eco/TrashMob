...
import {
    ...
    GetPartnerLocationServiceByLocationIdAndServiceType,
...
export const PartnerLocationServices: React.FC<PartnerLocationServicesDataProps> = (props) => {
    ...
    const getPartnerLocationsServicesByLocationId = useQuery({
        queryKey: GetPartnerLocationsServicesByLocationId({
            locationId: props.partnerLocationId,
        }).key,
        queryFn: GetPartnerLocationsServicesByLocationId({
            locationId: props.partnerLocationId,
        }).service,
        staleTime: Services.CACHE.DISABLE,
        enabled: false,
    ...
    function editService(serviceTypeId: number) {
        getPartnerLocationServiceByLocationIdAndServiceType
            .mutateAsync({ locationId: props.partnerLocationId, serviceTypeId })
            .then((res) => {
    ...
    function removeService(serviceTypeId: number) {
        if (
            !window.confirm(
                `Please confirm that you want to remove service type: '${getServiceType(
                    serviceTypeList,
                    serviceTypeId,
                )}' from this Partner?`,
            )
    ...
    React.useEffect(() => {
        if (notes === '' || notesErrors !== '' || serviceTypeId === 0) {
            setIsSaveEnabled(false);
        } else {
    ...
    const locationServiceActionDropdownList = (serviceTypeId: number) => (
        <>
            <Dropdown.Item onClick={() => editService(serviceTypeId)}>
                <Pencil />
                Edit Service
            </Dropdown.Item>
            <Dropdown.Item onClick={() => removeService(serviceTypeId)}>
                <XSquare />
                Remove Service
            </Dropdown.Item>
        </>
    ...
    function renderPartnerLocationServicesTable(services: PartnerLocationServiceData[]) {
        return (
            <div>
                ...
                <Button disabled={!isAddEnabled} className='action' onClick={() => addService()}>
                    Add Service
                </Button>
            </div>
        );
                        ...
                        <Col>
                            <Form.Group>
                                <OverlayTrigger placement='top' overlay={renderServiceTypeToolTip}>
                                    <Form.Label className='control-label font-weight-bold h5' htmlFor='ServiceType'>
                                        Service Type
                                    </Form.Label>
                                </OverlayTrigger>
                                <div>
                                    <select
                                        ...
                                        onChange={(val) => selectServiceType(val.target.value)}
                                        required
                                    >
                            ...
                            <Form.Group>
                                <div>
                                    <OverlayTrigger placement='top' overlay={renderIsAdvanceNoticeRequiredToolTip}>
                                        <Form.Label
                                            className='control-label font-weight-bold h5'
                                            htmlFor='isAdvanceNoticeRequired'
                                        >
    ...
    return (
        <div className='bg-white p-5 shadow-sm rounded'>
            ...
            {!isPartnerLocationServicesDataLoaded && props.partnerLocationId !== Guid.EMPTY && (
                <p>
                    <em>Loading...</em>
                </p>
            )}
            {isPartnerLocationServicesDataLoaded && partnerLocationServices
                ? renderPartnerLocationServicesTable(partnerLocationServices)
                : null}
            {isEdit || isAdd ? renderAddPartnerService() : null}
        </div>
    );
};
