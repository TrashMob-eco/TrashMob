...
import {
  GetEventPickupLocationsByUser,
  PickupLocationMarkAsPickedUp,
...
const MyDashboard: FC<MyDashboardProps> = (props) => {
  ...
  const [partnerStatusList, setPartnerStatusList] = useState<
    PartnerStatusData[]
  >([]);
  const [partnerRequestStatusList, setPartnerRequestStatusList] = useState<
    PartnerRequestStatusData[]
  >([]);
  ...
  const [myPartnerAdminInvitations, setMyPartnerAdminInvitations] = useState<
    DisplayPartnerAdminInvitationData[]
  >([]);
  const [myPickupRequests, setMyPickupRequests] = useState<
    PickupLocationData[]
  >([]);
  const [isEventDataLoaded, setIsEventDataLoaded] = useState<boolean>(false);
  const [
    isPartnerAdminInvitationsDataLoaded,
    setIsPartnerAdminInvitationsDataLoaded,
  ] = useState<boolean>(false);
  const [isPickupRequestsDataLoaded, setIsPickupRequestsDataLoaded] =
    useState<boolean>(false);
  ...
  const [upcomingEventsMapView, setUpcomingEventsMapView] =
    useState<boolean>(false);
  ...
  const getPartnerAdminInvitationsByUser = useQuery({
    queryKey: GetPartnerAdminInvitationsByUser({ userId: props.currentUser.id })
      .key,
    queryFn: GetPartnerAdminInvitationsByUser({ userId: props.currentUser.id })
      .service,
    staleTime: Services.CACHE.DISABLE,
    enabled: false,
  });
  ...
  const getEventPickupLocationsByUser = useQuery({
    queryKey: GetEventPickupLocationsByUser({ userId: props.currentUser.id })
      .key,
    queryFn: GetEventPickupLocationsByUser({ userId: props.currentUser.id })
      .service,
    staleTime: Services.CACHE.DISABLE,
    enabled: false,
  });
  ...
  const getPartnerRequestByUserId = useQuery({
    ...
    queryFn: GetPartnerRequestByUserId({ userId: props.currentUser.id })
      .service,
    staleTime: Services.CACHE.DISABLE,
    enabled: false,
  });
  ...
  const setSharingEvent = useCallback(
    (newEventToShare: EventData, updateShowModal: boolean) => {
  ...
  const handleCopyLink = (eventId: string) => {
    navigator.clipboard.writeText(
      window.location.origin + "/eventdetails/" + eventId
    );
    setCopied(true);
    setTimeout(() => {
      setCopied(false);
  ...
  const handleAcceptInvitation = (partnerAdminInvitationId: string) => {
    acceptPartnerAdminInvitation
      .mutateAsync({ invitationId: partnerAdminInvitationId })
      .then(() => {
        setIsPartnerAdminInvitationsDataLoaded(false);
        getPartnerAdminInvitationsByUser
          .refetch()
          .then((partnerAdminInvitationRes) => {
            setMyPartnerAdminInvitations(
              partnerAdminInvitationRes.data?.data || []
            );
            setIsPartnerAdminInvitationsDataLoaded(true);
            getPartnerAdminsForUser
              .refetch()
  ...
  const handleUnregisterEvent = (id: string, name: string) => {
    if (
      !window.confirm(
        "Do you want to remove yourself from this event: " + name + "?"
      )
    )
      return;
    else {
      deleteEventAttendee
        .mutateAsync({ eventId: id, userId: props.currentUser.id })
        .then(() => {
          handleReloadEvents();
        });
    }
  ...
  const partnerAdminInvitationsActionDropdownList = (
    partnerAdminInvitationId: string
  ) => {
    return (
      <>
        <Dropdown.Item
          onClick={() => handleAcceptInvitation(partnerAdminInvitationId)}
        >
          <CheckSquare />
          Accept Invitation
        </Dropdown.Item>
        <Dropdown.Item
          onClick={() => handleDeclineInvitation(partnerAdminInvitationId)}
        >
          <XSquare />
          Decline Invitation
        </Dropdown.Item>
  ...
  const pickupRequestActionDropdownList = (
  ...
  const UpcomingEventsTable = () => {
    ...
    return (
      <div className="bg-white p-0 p-sm-3 px-0 px-sm-4 overflow-auto">
                  ...
                  <tr key={event.id.toString()}>
                    ...
                    <td className="btn py-0">
                      <Dropdown role="menuitem">
                        ...
                        <Dropdown.Menu id="share-menu">
                          {event.createdByUserId === props.currentUser.id
                            ? completedEventOwnerActionDropdownList(event.id)
                            : completedAttendeeActionDropdownList(event.id)}
                        </Dropdown.Menu>
                      </Dropdown>
                    </td>
                  </tr>
    ...
    if (myPartners) {
                      ...
                      <Dropdown role="menuitem">
                        ...
                        <Dropdown.Menu id="share-menu">
                          ...
                          PartnerStatusActive
                            ...
                            : inactivePartnerActionDropdownList(
                                displayPartner.id
                              )}
                        </Dropdown.Menu>
                      </Dropdown>
      ...
      return (
        <div className="bg-white p-3 px-4">
          <Table columnHeaders={headerTitles}>No Events</Table>
        </div>
      );
    }
  };
                ...
                <tr key={displayPickup.id.toString()}>
                  ...
                  <td className="btn py-0">
                    <Dropdown role="menuitem">
                      ...
                      <Dropdown.Menu id="share-menu">
                        {pickupRequestActionDropdownList(
                          displayPickup.id,
                          displayPickup.eventId
                        )}
                      </Dropdown.Menu>
                    </Dropdown>
                  </td>
  ...
  const PartnerAdminInvitationsTable = () => {
    ...
    if (isPartnerAdminInvitationsDataLoaded && myPartnerAdminInvitations) {
      return (
        <div className="bbg-white p-0 p-sm-3 px-0 px-sm-4 overflow-auto">
          <Table columnHeaders={headerTitles}>
                ...
                <tr key={displayInvitation.id.toString()}>
                  ...
                  <td className="btn py-0">
                    <Dropdown role="menuitem">
                      ...
                      <Dropdown.Menu id="share-menu">
                        {partnerAdminInvitationsActionDropdownList(
                          displayInvitation.id
                        )}
                      </Dropdown.Menu>
                    </Dropdown>
                  </td>
  ...
  return (
    <>
      <HeroSection
        Title="Dashboard"
        Description="See how much you've done!"
      ></HeroSection>
      <Container className="mt-5 pb-5">
        {eventToShare && (
          <SocialsModal
            ...
            message={SharingMessages.getEventShareMessage(
              eventToShare,
              props.currentUser.id
            )}
          />
          ...
          <Col>
            <div className="d-flex bg-white">
              <Col className="ml-3">
                ...
                <p className="card-statistic color-primary mt-0">
                  {totalEvents}
                </p>
              </Col>
      ...
      <Container className="mb-5 pb-5">
        <div className="d-flex my-5 mb-4 justify-content-between">
          <h4 className="font-weight-bold mr-2 pb-2 mt-0 active-line">
              ...
              <div className="d-flex align-items-center mt-4">
                <label className="mr-2">
                  <input
                    ...
                    value="list"
                    onChange={(e) =>
                      handleEventView(e.target.value, "Upcoming events")
                    }
            ...
            <>
              <AzureMapsProvider>
                <MapControllerPointCollection
                  forceReload={false}
                  center={center}
                  multipleEvents={myEventList.filter(
                    (event) => new Date(event.eventDate) >= new Date()
                  )}
            ...
            <div className="d-flex align-items-center mt-4">
              <label className="mr-2">
                <input
                  ...
                  value="list"
                  onChange={(e) =>
                    handleEventView(e.target.value, "Past events")
                  }
