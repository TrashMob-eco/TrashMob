...
export const UserEvents: React.FC<UserEventsPropsType> = (props) => {
                ...
                <table className='table table-striped' aria-labelledby='tableLabel'>
                    ...
                    <tbody>
                        {events
                            ...
                            .map((mobEvent) => {
                                const isOwner = mobEvent.createdByUserId === props.currentUser.id;
                                    ...
                                    <tr key={mobEvent.id.toString()}>
                                        ...
                                        <td>
                                            <Button
                                                ...
                                                className='action'
                                                onClick={() =>
                                                    props.history.push(`/manageeventdashboard/${mobEvent.id}`)
                                                }
                                            >
                                                Manage Event
                                            </Button>
                                            <Button
                                                hidden={!isOwner}
                                                className='action'
                                                onClick={() => props.history.push(`/cancelevent/${mobEvent.id}`)}
                                            >
                                                Cancel Event
                                            </Button>
                                            <Button
                                                className='action'
                                                onClick={() => props.history.push(`/eventdetails/${mobEvent.id}`)}
                                            >
                                                View Details
                                            </Button>
                                            <Button
                                                className='action'
                                                onClick={() => props.history.push(`/eventsummary/${mobEvent.id}`)}
                                            >
                                                View Summary
                                            </Button>
                                            <Button
                                                hidden={isOwner}
                                                className='action'
                                                onClick={() => handleRemove(mobEvent.id, mobEvent.name)}
                                            >
    ...
    const contents = props.isEventDataLoaded ? (
