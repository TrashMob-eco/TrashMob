...
import { GetWaiverVersionById, UpdateWaiverVersion, GetAllWaiverVersions } from '@/services/waiver-admin';
import { WaiverScope, WaiverVersionData } from '@/components/Models/WaiverVersionData';
...
export const SiteAdminWaiverEdit = () => {
    const { currentUser } = useLogin();
    const { waiverId } = useParams<{ waiverId: string }>() as { waiverId: string };

    const { data: waiver } = useQuery({
        queryKey: GetWaiverVersionById({ id: waiverId }).key,
        queryFn: GetWaiverVersionById({ id: waiverId }).service,
        select: (res) => res.data,
    });
    ...
    const updateWaiver = useMutation({
        mutationKey: UpdateWaiverVersion().key,
        mutationFn: UpdateWaiverVersion().service,
    ...
    useEffect(() => {
        if (waiver) {
            form.reset({
                ...
                scope: waiver.scope,
                effectiveDate: waiver.effectiveDate ? new Date(waiver.effectiveDate).toISOString().split('T')[0] : '',
                expiryDate: waiver.expiryDate ? new Date(waiver.expiryDate).toISOString().split('T')[0] : '',
    ...
    const isSubmitting = updateWaiver.isPending;
