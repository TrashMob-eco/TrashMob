...
export const EditArea = () => {
    ...
    const { slug, areaId } = useParams<{ slug: string; areaId: string }>() as { slug: string; areaId: string };
    ...
    >({
        ...
        select: (res) => res.data,
        enabled: !!community?.id && !!areaId,
    });

    const { mutate, isPending: isSubmitting } = useMutation({
        mutationKey: UpdateAdoptableArea().key,
        mutationFn: (body: AdoptableAreaData) =>
            UpdateAdoptableArea().service({ partnerId: community?.id || '', areaId }, body),
        onSuccess: () => {
            ...
            navigate(`/communities/${slug}/admin/areas`);
        },
        onError: () => {
            toast({
                variant: 'destructive',
    ...
    const onSubmit: SubmitHandler<FormInputs> = useCallback(
        ...
        [community?.id, area, mutate],
    );

    if (communityLoading || areaLoading) {
        return (
        ...
        <div className='container py-8'>
            <div className='mb-6'>
                <Button variant='ghost' size='sm' asChild>
                    <Link to={`/communities/${slug}/admin/areas`}>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
                    ...
                    <div className='flex justify-end gap-2'>
                        <Button
                            type='button'
                            variant='outline'
                            onClick={() => navigate(`/communities/${slug}/admin/areas`)}
                        >
                            Cancel
                        </Button>
