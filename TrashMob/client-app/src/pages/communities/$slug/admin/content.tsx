...
export const CommunityContentEdit = () => {
    const queryClient = useQueryClient();
    const { slug } = useParams<{ slug: string }>() as { slug: string };
    const { toast } = useToast();

    // First get the community by slug to get its ID
    ...
    // Then get the full community data for admin editing
    const { data: currentValues, isLoading } = useQuery<AxiosResponse<CommunityData>, unknown, CommunityData>({
        queryKey: GetCommunityForAdmin({ communityId: community?.id || '' }).key,
        queryFn: GetCommunityForAdmin({ communityId: community?.id || '' }).service,
        select: (res) => res.data,
...
export default CommunityContentEdit;
