...
export const PartnerCommunitySponsorEdit = () => {
    const navigate = useNavigate();
    const queryClient = useQueryClient();
    const { partnerId, sponsorId } = useParams<{ partnerId: string; sponsorId: string }>() as {
        partnerId: string;
        sponsorId: string;
    };
    const { toast } = useToast();

    const { data: sponsor, isLoading } = useQuery<AxiosResponse<SponsorData>, unknown, SponsorData>({
        queryKey: GetSponsor({ partnerId, sponsorId }).key,
        queryFn: GetSponsor({ partnerId, sponsorId }).service,
        select: (res) => res.data,
        enabled: !!partnerId && !!sponsorId,
    });

    const { mutate, isPending: isSubmitting } = useMutation({
        mutationKey: UpdateSponsor().key,
        mutationFn: (body: SponsorData) => UpdateSponsor().service({ partnerId, sponsorId }, body),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: GetSponsors({ partnerId }).key });
    ...
    useEffect(() => {
        if (sponsor) {
            form.reset({
                ...
                showOnPublicMap: sponsor.showOnPublicMap,
    ...
    const onSubmit: SubmitHandler<FormInputs> = useCallback(
        (formValues) => {
            if (!partnerId || !sponsor) return;

            const updated: SponsorData = {
                ...sponsor,
                                ...
                                render={({ field }) => (
                                    <FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                                        <div className='space-y-0.5'>
                                            ...
                                            <FormDescription>
                                                Inactive sponsors are hidden but their data is preserved.
                                            </FormDescription>
                                        </div>
                                        <FormControl>
                                            <Switch checked={field.value} onCheckedChange={field.onChange} />
...
export default PartnerCommunitySponsorEdit;
