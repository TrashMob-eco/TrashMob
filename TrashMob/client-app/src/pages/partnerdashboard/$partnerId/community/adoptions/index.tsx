...
export const PartnerCommunityAdoptions = () => {
    ...
    >({
        ...
        queryFn: GetPendingApplications({ partnerId }).service,
        select: (res) => res.data,
        enabled: !!partnerId,
    });
    ...
    const { data: approvedAdoptions, isLoading: approvedLoading } = useQuery<
        ...
        TeamAdoptionData[]
    >({
        queryKey: GetApprovedAdoptions({ partnerId }).key,
        queryFn: GetApprovedAdoptions({ partnerId }).service,
        select: (res) => res.data,
        enabled: !!partnerId,
    });
    ...
    const { data: delinquentAdoptions, isLoading: delinquentLoading } = useQuery<
        ...
        TeamAdoptionData[]
    >({
        queryKey: GetDelinquentAdoptions({ partnerId }).key,
        queryFn: GetDelinquentAdoptions({ partnerId }).service,
        select: (res) => res.data,
        enabled: !!partnerId,
    });
    ...
    >({
        queryKey: GetComplianceStats({ partnerId }).key,
        queryFn: GetComplianceStats({ partnerId }).service,
        select: (res) => res.data,
        enabled: !!partnerId,
    });
    ...
    const handleApprove = useCallback(
        (adoptionId: string) => {
            if (!partnerId) return;
            approveAdoption({ partnerId, adoptionId });
        },
        [partnerId, approveAdoption],
    );
    ...
    const handleExport = useCallback(async () => {
        if (!partnerId) return;
        try {
            const response = await ExportAdoptions({ partnerId }).service();
            const blob = response.data;
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `Adoptions_${new Date().toISOString().split('T')[0]}.csv`;
...
export default PartnerCommunityAdoptions;
